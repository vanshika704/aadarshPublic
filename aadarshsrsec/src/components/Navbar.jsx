...
import React, { useState, useEffect } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { Edit, Plus, Trash2, X, Save, Loader2, FileUp, Settings, Phone } from 'lucide-react';
...
import { 
  ...
  updateDossierData, 
  uploadFile,
  getTopBarData, // Ensure these are exported in your service
  updateTopBarData 
...
// --- Sub-Component: Top Bar Settings Editor ---
const TopBarEditor = ({ onClose, currentData, onSave }) => {
  const [data, setData] = useState(currentData);
  const [loading, setLoading] = useState(false);

  const handlePdfUpload = async (file) => {
    if (!file) return;
    const toastId = toast.loading("Uploading new disclosure PDF...");
    try {
      const url = await uploadFile(file, "topbar_docs");
      setData({ ...data, disclosurePath: url });
    ...
    <div className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="bg-red-700 p-4 text-white flex justify-between items-center">
          <h3 className="font-bold flex items-center gap-2"><Settings size={18}/> Top Bar Settings</h3>
          <button onClick={onClose}><X size={20}/></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="text-[10px] font-bold text-gray-400 uppercase">Marquee Text</label>
            <input className="w-full border-b py-2 outline-none focus:border-red-500 text-sm" 
              value={data.marqueeText} onChange={e => setData({...data, marqueeText: e.target.value})} />
          </div>
          <div>
          ...
          <div className="bg-gray-50 p-3 rounded border">
            <label className="text-[10px] font-bold text-gray-400 uppercase">Mandatory Disclosure PDF</label>
            <div className="flex items-center gap-2 mt-2">
               <input className="flex-1 bg-white border rounded px-2 py-1 text-[10px] text-gray-400 truncate" value={data.disclosurePath} readOnly />
               <label className="cursor-pointer bg-red-700 text-white p-1.5 rounded hover:bg-red-800">
                  <FileUp size={14} />
        ...
        <div className="p-4 bg-gray-50 border-t flex justify-end gap-3">
          ...
          <button onClick={async () => { setLoading(true); await onSave(data); setLoading(false); onClose(); }}
            className="bg-red-700 text-white px-6 py-2 rounded font-bold flex items-center gap-2 text-sm">
            {loading ? <Loader2 className="animate-spin" size={16}/> : <Save size={16}/>} Save Bar
          </button>
        </div>
      </div>
    </div>
...
function Navbar({ admissionYear = "2025-2026" }) {
  ...
  const [topBar, setTopBar] = useState({ marqueeText: `Admissions Open ${admissionYear}`, phone: "79882-12029", disclosurePath: "/mandatory-public-disclosure_compressed.pdf" });
  const [menuItems, setMenuItems] = useState(INITIAL_MENU_DATA);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  ...
  useEffect(() => {
    const fetchNavData = async () => {
      try {
        const [dbActivities, dbDossier, dbTopBar] = await Promise.all([
          getActivitiesData(),
          getDossierData(),
          getTopBarData()
        ]);
        
        if (dbTopBar) setTopBar(dbTopBar);
  ...
  const handleUpdateTopBar = async (newData) => {
    const toastId = toast.loading("Updating Top Bar...");
    try {
      await updateTopBarData(newData);
      setTopBar(newData);
      toast.success("Top Bar updated!", { id: toastId });
    } catch (err) { toast.error("Failed to update.", { id: toastId },err); }
  };
  ...
  const handleSaveActivities = async (newActivitiesList) => {
    ...
    } catch (error) { toast.error("Failed to save.", { id: toastId },error); }
  };
  ...
  const handleSaveDossier = async (newList) => {
    ...
    } catch (err) { toast.error("Failed to update Dossier.", { id: toastId },err); }
  };
  ...
  return (
    <div className="font-sans relative">
      {/* --- TOP UTILITY BAR (EXACT UI) --- */}
      <div className={`${BRAND_COLOR} text-white text-[10px] md:text-[11px] font-medium py-1 px-4 overflow-hidden relative`}>
        <style>{`
          @keyframes marquee { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } }
          .animate-marquee-text { animation: marquee 15s linear infinite; white-space: nowrap; display: inline-block; }
          .animate-marquee-text:hover { animation-play-state: paused; }
        `}</style>

        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-1 md:gap-0">
          <div className="flex gap-3 tracking-wide uppercase z-10 bg-red-700 md:bg-transparent pr-2 items-center">
            <a href={topBar.disclosurePath} target="_blank" rel="noopener noreferrer" className="hover:underline hover:text-red-100 cursor-pointer flex items-center gap-1">
              <span>Mandatory Disclosure</span>
              <svg className="w-3 h-3 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
            </a>
            <span className="hidden md:inline text-white/50">|</span>
            <span className="hidden md:inline">Co-Ed English Medium</span>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 w-full md:w-auto">
            ...
            <div className="flex items-center gap-2 z-10 bg-red-700 pl-2">
              <a href={`tel:${topBar.phone}`} className="whitespace-nowrap hover:text-red-200 transition-colors flex items-center gap-1">
                <Phone size={10} className="inline md:hidden" /> {topBar.phone}
              </a>
              {isAdmin && (
                <button onClick={() => setIsEditingTopBar(true)} className="p-1 bg-white/20 rounded hover:bg-white/40 ml-1">
                  <Settings size={12} />
                </button>
              )}
            </div>
      ...
      {/* --- EDIT MODALS --- */}
      {isEditingTopBar && <TopBarEditor currentData={topBar} onClose={() => setIsEditingTopBar(false)} onSave={handleUpdateTopBar} />}
      {isEditingActivities && <ActivitiesEditor onClose={() => setIsEditingActivities(false)} currentActivities={menuItems.find(m => m.title === "Activities")?.items} onSave={handleSaveActivities} />}
      {isEditingDossier && <DossierEditor onClose={() => setIsEditingDossier(false)} currentItems={menuItems.find(m => m.title === "Dossier")?.items} onSave={handleSaveDossier} />}

      {/* --- STICKY NAVBAR (EXACT UI) --- */}
                ...
                return (
                  <div key={index} className="group relative h-full flex items-center">
                    ...
                    {isAdmin && (menu.title === "Activities" || menu.title === "Dossier") && (
                        <button onClick={(e) => { e.stopPropagation(); menu.title === "Activities" ? setIsEditingActivities(true) : setIsEditingDossier(true); }} className="p-1 text-gray-400 hover:text-blue-600 transition-colors">
                          <Edit size={12} />
                        </button>
                    )}
